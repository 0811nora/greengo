const OrderList = ({
				...
				<div className="d-flex flex-column h-100">
					...
					<div className="flex-grow-1 overflow-y-auto custom-scrollbar">
						...
						<table className="table table-borderless align-middle mb-0 ">
							...
							<tbody>
								{categoryTab
									.filter(
										(tab) => tab !== "addOn" && selectedProduct[tab].length > 0,
									)
									.map((tab) =>
										selectedProduct[tab].map((p, index) => (
											<tr key={`${tab}-${index}`}>
												...
												<td className="text-end">
													{tab === "protein"
														? (p.price - discount) * p.qty > 0
															? `$${(p.price - discount) * p.qty}`
															: "-"
														: "-"}
												</td>
											</tr>
						...
						<div className=" mt-2 mb-4 px-2 bg-primary-100 py-2">
							...
							<div className="d-flex justify-content-between">
								...
								<p className="fw-bold text-dark">
									${totalProteinPrice.toLocaleString()}
								</p>
							</div>
						</div>
						...
						{selectedProduct["addOn"].length > 0 && (
							<div className="mt-4">
								<h6 className="mb-3 text-center text-gray-500 fw-bold border-bottom pb-2">
									加購品項
								</h6>
					...
					<div className="flex-shrink-0 pt-3 border-top mt-3">
						<div className="d-flex justify-content-between align-items-center">
							...
							<span className="fs-4 fw-bold text-price">
	...
	return (
		<div
			className={`${styleType} p-0 d-flex flex-column h-100 overflow-hidden`}
		>
			{/* 卡片標題區  */}
			<div className="flex-shrink-0 p-4 pb-2">
				<div className="row g-0 border-bottom pb-3">
					<div className="col-7 ps-2">
						<div className="d-flex align-items-center">
							...
							<h6 className="mb-0 fw-bold text-dark">
								{orderTitle}{" "}
								<span className="text-primary small ms-1">#自由配</span>
							</h6>
						</div>
					</div>
					<div className="col-5 ps-4 border-start d-none d-lg-block">
						<div className="d-flex align-items-center">
							<div
								className="bg-orange-400 rounded-pill me-2"
								style={{
									width: "4px",
									height: "18px",
			...
			{/* 2. list區塊 */}
			<div
				className="flex-grow-1 overflow-y-auto custom-scrollbar px-4"
				style={{ minHeight: 0 }}
			>
					...
					<div className="col-12 col-lg-7 pe-lg-3 py-2">
						{categoryTab
							...
							.map((tab) => (
								<div
									key={tab}
									className="mb-3 bg-light rounded-3 p-3 border border-white shadow-sm"
								>
									<div className="d-flex justify-content-between gap-lg-3 align-items-center">
										<div className="d-flex align-items-start gap-3 flex-grow-1">
											<div style={{ minWidth: "65px" }}>
												<span
													className="badge fs-md rounded-pill bg-white text-primary border border-primary-subtle px-2 py-2 w-100"
										...
										{isEdit && (
											<button
												className="btn btn-sm btn-white border border-gray-200 shadow-sm rounded-circle ms-3 flex-shrink-0"
							...
							<div className="d-flex flex-column justify-content-between h-100">
								<div className="list-group list-group-flush ">
									{selectedProduct["addOn"].map((p, i) => (
										<div
											key={i}
											className="list-group-item bg-transparent border-0 px-0 py-2 d-flex justify-content-between align-items-center"
										>
								...
								<div className="text-center mb-4">
									<button
										className="btn btn-sm btn-outline-primary w-50 mt-3"
										onClick={onAddonEdit}
									>
										編輯加購品項
									</button>
