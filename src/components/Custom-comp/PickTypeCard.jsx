const PickTypeCard = ({
  ...
  const pickType = (e) => {
    const targetData = sourceData.find(
      (item) => item.content.plan_type === e.target.value,
    );
    ...
    <div className="form-check p-2">
      ...
      >
        <div className="position-relative">
          <div
            className={`step1-card ${selectedValue === typeText ? "c-active-card" : ""}`}
            style={{ backgroundImage: `url(${bgImage})` }}
          >
            <div className="dark-overlay">
              <div className="d-flex card-outside">
                <div className="title-area align-self-md-end align-self-start">
                  <h2 className="step-title text-white text-shadow">{title}</h2>
                </div>
                <div className=" card-content py-8  flex-column align-items-end dark-overlay2">
                  ...
                  <ul
                    className="w-100 p-5 text-gray-400 fw-medium "
          ...
          <div
            className={`position-absolute top-0 start-0 c-pick-btn ${selectedValue === typeText ? "c-active-pick-btn" : ""}`}
          >
