...
export default function AdminHeader({ admMode, handleToggleMode, handleNavMode, admSignout }) {
      ...
      <nav className="navbar fixed-top">
        <div className="container adm__navbar adm__glassbg ">
          ...
          <ul className="navbar-nav delete__navbar_nav__glassbg">
            ...
            {admNavbarData.navLink_needAdmMode.map(item => (
              <li className="nav-item" key={item.route}>
                <NavLink
                  className={({ isActive }) =>
                    `d-flex aligns-item-center nav-link adm__navLink ${admMode ? '' : 'disable'
                    } ${isActive ? 'active' : ''}`
                  }
                  to={item.route}
                  onClick={e => {
          ...
          <div className="d-flex align-items-center gap-4">
            ...
            {/* 登出按鈕 */}
            <button type="button" className="btn adm__button__secondary  sm" onClick={() => {
              admSignout();
            }}>
              登出
            </button>
