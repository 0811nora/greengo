...
export default function Article() {
  ...
  const renderTypeArticle = hasMore
    ? filteredArticles.slice(
        0,
        showCount === 0 && filteredArticles.length > 0
        ...
        <main className="article_main">
          ...
          <section className="position-relative banner">
            ...
            <div className="position-relative z-1 d-flex flex-column pt-8 justify-content-center align-items-center h-100">
              <h1 className="text-white fw-bold display-lg-4  fs-2 mb-3">
                綠果專欄
              </h1>
              <p
                className="fs-lg-5 fs-6 text-white"
          ...
          <section className="article-nav">
            <ul className="  bg-gray-400 d-flex justify-content-lg-center justify-content-start align-items-center ">
              <li>
                <button
                  className={`nav-filter-btn ${activeTab === "全部" ? "active" : ""}`}
                  onClick={handleArticleType}
                  type="button"
                >
                  全部
                </button>
              ...
              {articleTypeList.map((item) => (
                <li key={item}>
                  <button
                    className={`nav-filter-btn ${activeTab === item ? "active" : ""}`}
                    onClick={handleArticleType}
                    type="button"
                  >
                    {item}
                        ...
                        <div className="article-card h-100 d-flex flex-column justify-content-between">
                          <div>
                            ...
                            <div className="position-relative">
                              ...
                              <div className="position-absolute bottom-0 start-0 m-2 ">
                                {data["tag"].map((i) => (
                                  <span
                                    key={i}
                                    className=" px-2 py-1 mx-2 rounded-1 bg-primary text-white shadow fs-sm"
                                  >
                                    # {i}
                                  </span>
                                ))}
                            ...
                            <div className="card-body pb-4 pt-6 px-3">
                              <h5 className="card-title mb-4 line-clamp-2">
                                {data.title}
                              </h5>
                              <p className="card-text line-clamp-2 text-brown-300">
                                {data.description}
                              </p>
                            </div>
                          </div>
