...
const ProteinTip = ({ mode, maxCount, activeTab }) => {
  return (
    <div
      className={`text-center ${mode === "pc" ? "d-lg-block d-none" : " d-lg-none"}`}
    >
      <p
        className={`w-100 text-brown-300 pb-5  ${mode === "pc" ? "position-absolute top-100 start-50 translate-middle " : ""}`}
      >
...
const AddonTypeBtnGroup = ({ addOnTabs, onAddonTab, mode }) => {
  return (
    <div className={mode === "pc" ? "d-lg-block d-none" : " d-lg-none"}>
      <div
        className={`text-center pb-lg-6 mb-lg-5 ${mode === "pc" ? "position-absolute top-100 start-50 translate-middle" : ""} `}
      >
            ...
            >
              <button
                className={`nav-link border-0 px-6 position-relative ${
                  addOnTabs === tab
                    ? "border-bottom border-3 border-primary active fw-bold"
                    : "text-muted"
                }`}
...
export default function Custom() {
  ...
  useEffect(() => {
    const getCustomProduct = async () => {
      try {
        ...
        notify('error', `取得失敗:${error.response.data.message}`,"bottom-center");
      }
    };
    getCustomProduct();
  }, []);
  ...
  const renderItemList = getAllitem.filter(
    (item) => item.product_type === activeTab,
  );
  const renderaddOnItemList = getAllitem.filter(
    (item) => item.product_type === addOnTabs,
  );
  ...
  // 計算類別的總數量
  const totalQty = (type) =>
    selectedProduct[type].reduce((sum, item) => sum + item.qty, 0);

  // 計算已選單品的數量
  const itemQty = (title) => {
    const target = selectedProduct[activeTab].find(
  ...
  // 計算所有已選品項的三大營養素加總
  totalNutrition = totalNutrition.reduce(
    (acc, currItem) => {
      return {
  ...
  const totalProteinPrice = findproteinPrice.reduce(
    (acc, item) => acc + item.totalPrice,
    0,
  );
  const totalAddonPrice = findAddonPrice.reduce(
    (acc, item) => acc + item.totalPrice,
    0,
  );
  ...
  const pickProduct = (e, price, category, id) => {
    ...
    if (checked) {
      ...
      const isExist = selectedProduct[name].some(
      ...
      setSelectedProduct((pre) => ({
        ...
        [name]: [
          ...pre[name],
          { id: id, title: value, qty: 1, price: price, type: category },
        ],
      }));
    } else {
      setSelectedProduct((pre) => ({
  ...
  const handleNextBtn = () => {
    if (stepState === 1) {
      ...
      const underTarget = requiredTabs.find(
        (item) => totalQty(item) !== maxCount[item],
      );
      ...
      if (underTarget) {
        ...
        notify(
          "info",
          `${titleName[underTarget]}數量不足，請先選購。`,
          "bottom-center",
        );
        scrollToTop();
      } else {
  ...
  const handleSendCart = async () => {
    ...
    try {
      ...
      notify('error', `加入購物車失敗:${error.response.data.message}`,"bottom-center");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const renderItemCards = (list, tabName) => {
    return list.map((item) => {
      ...
      const isLightProtein =
        selectedProduct.plan_type === "light" && tabName === "protein";
      ...
      const isChecked =
        tabName === "addOn"
          ...
          : selectedProduct[tabName]?.some((i) => i.title === item.title) ||
            false;
      ...
      return (
        ...
        >
          <ItemCard
            ...
            onChange={(e) =>
              pickProduct(e, item.price, item.product_type, item.id)
            }
            onClickPlus={() =>
              editQty(item.title, 1, item.price, item.product_type, item.id)
            }
            onClickMins={() =>
              editQty(item.title, 0, item.price, item.product_type, item.id)
            }
            detail={{
              grams: item.grams,
                  ...
                  >
                    <section className=" py-5 text-center tracking-in-contract overflow-hidden">
                      ...
                      <AdmButton
                        ...
                        size={"lg"}
                        className={
                          "btn-primary py-2 px-7 px-lg-8  fs-lg-6 rounded-5 wobble-hor-bottom"
                        }
                      />
                    </section>
                  </div>
                        ...
                        <div className=" h-100 d-flex flex-column flex-lg-row justify-content-center align-items-center justify-content-lg-between mb-8 c-mt-lg-60 mt-6">
                          ...
                          >
                            <h2 className=" mb-lg-6 fs-lg-1 fs-2 py-4">
                              <span className="  mb-3 text-orange-10">
                                Step1{" "}
                              </span>
                              <br />
                              <span className=" text-primary">選擇套餐</span>
                            </h2>
                            <p className="text-brown-300  fw-bold">
                              選擇今日最適合你的飲食偏好
                            </p>
                          </div>
                      ...
                      <section className=" h-100  pt-10 pb-6 py-lg-4   px-lg-8 px-3  d-flex flex-column">
                        <div className="flex-shrink-0 text-center text-lg-start mb-lg-3 position-relative  ">
                          <h2 className=" mb-lg-3 fs-2 pt-6 pb-3 ">
                            <span className="  mb-2 text-orange-10">
                              Step2{" "}
                            </span>
                            <br />
                            <span className=" text-primary">開始自由配</span>
                          </h2>
                          <p className="text-brown-300 fw-bold mb-4 mb-lg-0">
                            請打造專屬於您的自由配餐點
                          </p>
                          ...
                          {activeTab === "addOn" && (
                            <AddonTypeBtnGroup
                              mode={"pc"}
                              addOnTabs={addOnTabs}
                              onAddonTab={(tab) => setAddonTabs(tab)}
                            />
                          )}
                        </div>
                        ...
                        <div className="flex-grow-1  mb-3 overflow-hidden ">
                          <div className="row h-100 g-0 m-0 flex-column flex-lg-row">
                              ...
                              <div className="d-flex  flex-lg-column gap-2 py-2 py-lg-2">
                                <div className="">
                                  <TypeListBtn
                                    ...
                                    num={
                                      <>
                                        {totalQty("base")}/{limit.base_limit}
                                      </>
                                    }
                                    imageUrl={baseIcon}
                                ...
                                <div className="">
                                  <TypeListBtn
                                    ...
                                    num={
                                      <>
                                        {totalQty("side")}/{limit.side_limit}
                                      </>
                                    }
                                    imageUrl={sideIcon}
                                ...
                                <div className="">
                                  <TypeListBtn
                                    ...
                                    num={
                                      <>
                                        {totalQty("sauce")}/{limit.sauce_limit}
                                      </>
                                    }
                                    imageUrl={sauceIcon}
                            ...
                            <div
                              className="col-lg-8  h-100 overflow-y-auto py-4 py-lg-0"
                          ...
                          <div className="d-none d-lg-block">
                            <ClickOutsideHandler
                              onOutsideClick={() => handleClose("order")}
                            >
                              {isOpenOrderModal && (
                                <OrderList
                                  onClose={() =>
                                  ...
                                  orderTitle={
                                    PLAN_NAME[selectedProduct.plan_type]
                                  }
                          ...
                          <div className="d-flex  justify-content-center  gap-3 mx-auto">
                            <div className="w-100 text-end">
                              <AdmButton
                                ...
                                size={"lg"}
                                className={
                                  "btn-gray-200 px-lg-10 py-2 w-100-mob rounded-5 "
                                }
                              />
                            </div>
                            <div className="w-100">
                              <AdmButton
                                ...
                                size={"lg"}
                                className={
                                  "btn-primary px-lg-10 py-2 w-100-mob rounded-5 "
                                }
                              />
                            </div>
                          </div>
                    ...
                    {stepState === 3 && (
                      <section className=" h-100  pt-10 pb-6 py-lg-4 px-lg-8 px-3  d-flex flex-column overflow-hidden">
                        ...
                        <div className="flex-grow-1 overflow-hidden">
                          <div
                            className="row flex-column flex-lg-row pb-2 h-100"
                            style={{ minHeight: 0 }}
                          >
                            <div className="col-1 d-none d-lg-block">
                              <button
                                className="btn btn-link text-brown-300 fs-3"
                                onClick={() => setStepState(stepState - 1)}
                                type="button"
                              >
                                ...
                                <div className="  mb-3 mt-lg-0 mt-4 px-2 px-lg-0">
                                  ...
                                  <div>
                                    <AdmButton
                                      ...
                                      size={"lg"}
                                      className={
                                        "btn-gray-200 d-lg-none px-lg-10 py-2 mb-3 w-100-mob rounded-5 "
                                      }
                                    />
                                    <AdmButton
                                      ...
                                      size={"lg"}
                                      className={
                                        "btn-primary  w-100 py-3  rounded-5"
                                      }
                                    />
                                  </div>
                                </div>
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="text-center mb-8">
                            ...
                            <h2 className="fw-bold text-dark mt-4 mb-4">
                              加入成功
                            </h2>
                            <p className="text-gray-500">
                              您的餐點已成功加入購物車，準備好要享用了嗎？
                            </p>
                          </div>
                          ...
                          <div className="d-flex flex-column gap-3 px-3 ">
                            <div className="d-flex justify-content-center gap-4">
                              <button
                                className="btn btn-primary  rounded-pill py-3 fw-bold shadow-sm d-flex align-items-center justify-content-center px-5"
                                onClick={() => {
                                  navigate("/cart");
                                }}
                                type="button"
                              ...
                              <button
                                className="btn btn-brown-300  rounded-pill py-3 fw-bold px-5 "
                                onClick={() => {
                                  (setStepState(0),
                                    setSelectedProduct(
                                      INITIAL_SELECTED_PRODUCT,
                            ...
                            <button
                              className="btn btn-link text-brown-300 mt-2 text-decoration-none fw-medium d-flex align-items-center justify-content-center"
                              onClick={() => {
                                navigate("/product");
                              }}
                              type="button"
                ...
                {stepState >= 2 && stepState < 4 && (
                  <>
                    <div className="c-nutrition-fab-wrapper d-lg-none">
                      <button
                        className={`btn ${isOpenPFCBtn ? "btn-primary" : "btn-brown-300"} c-nutrition-fab-btn`}
                        onClick={() => {
                          setIsOpenPFCBtn((prev) => !prev);
                        }}
                        type="button"
                      >
                        營養
                      ...
                      {isOpenPFCBtn && (
                        <ClickOutsideHandler
                          onOutsideClick={() => handleClose("pfc")}
                        >
                          <CompleteNutrition
                            calcPFC={pfcRatio()}
                            finalTotalNutrition={finalTotalNutrition}
